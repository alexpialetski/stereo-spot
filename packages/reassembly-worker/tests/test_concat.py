...
from reassembly_worker.concat import build_concat_list_paths, concat_segments_to_file
...
def test_build_concat_list_paths_orders_by_segment_index(
    ...
    # query_by_job returns ordered by segment_index, so 0, 1, 2
    ...
    # First call: out bucket, jobs/job-1/segments/0.mp4 from output_s3_uri
...
def test_concat_segments_to_file_writes_list_and_calls_ffmpeg(
    ...
    with patch("reassembly_worker.concat.subprocess.run") as mock_run:
        # Make ffmpeg "succeed" and touch output
