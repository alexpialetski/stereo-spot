"""
Reassembly worker loop: receive job_id, acquire lock, concat segments, upload final, update Job.
"""
...
def _parse_message_body(body: str | bytes) -> ReassemblyPayload | None:
...
def process_one_message(
    ...
    """
    payload = _parse_message_body(payload_str)
    if payload is None:
        return False
    ...
    while True:
        ...
        for msg in messages:
            ...
            try:
                ...
                # Delete on success or when lock not acquired (another worker has it)
                if ok:
