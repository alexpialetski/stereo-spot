...
Inference container using **iw3** ([nunif](https://github.com/nagadomi/nunif)): converts 2D video segments to stereoscopic 3D (side-by-side or anaglyph). Storage and metrics are adapter-based (AWS/GCP) so the same image can run on SageMaker or future GCP (e.g. Vertex AI).

## Contract

- **GET /ping** — returns 200 OK (SageMaker health check).
- **POST /invocations** — body: JSON `{"input_uri"|"s3_input_uri": "...", "output_uri"|"s3_output_uri": "...", "mode": "anaglyph"|"sbs"}`. The `mode` is optional and defaults to `"anaglyph"`. URIs can be `s3://` (AWS) or `gs://` (GCP). The handler downloads the segment via the storage adapter, runs **iw3** (2D→stereo), and uploads the result via the storage adapter. Metrics (e.g. conversion duration) are emitted via the metrics adapter when configured.

## Adapters

- **Storage:** `STORAGE_PROVIDER=aws|gcp` or infer from URI scheme (`s3://` → AWS, `gs://` → GCP). AWS uses S3 (boto3); GCP uses Cloud Storage.
- **Metrics:** `METRICS_PROVIDER=aws|gcp|none`. Default `aws` (CloudWatch). Use `none` for local/dev. GCP is no-op for now.

## Pipeline

The container runs **iw3** from the [nunif](https://github.com/nagadomi/nunif) repo:

- **Input:** Segment video from storage (S3 or GCS per URI).
...
- **Output:** Single stereo MP4 written to the given output URI.

Pre-trained models are **baked into the image** at build time via `python -m iw3.download_models`; no network or Secrets Manager at startup.
...
|---------------------|-------------|
| `STORAGE_PROVIDER`  | Optional. `aws` (default for s3://) or `gcp`. If unset, inferred from request URI scheme. |
| `METRICS_PROVIDER`  | Optional. `aws` (default), `gcp`, or `none`. |
...
Additional iw3 options are documented in [docs/RUNBOOKS.md](../../docs/RUNBOOKS.md#optional-iw3-tuning-sagemaker-inference-container) and can be wired in via env or request body later:
