...
def test_process_one_chunking_message_mock_flow(
    mock_job_store: MagicMock,
    mock_storage: MagicMock,
    tmp_path: Path,
) -> None:
    """With chunk_video_to_temp patched to return two fake segments, process_one_chunking_message
    ...
    with patch(
        "media_worker.chunking.chunk_video_to_temp",
        return_value=(fake_segments, FakeTmp()),
    ):
        result = process_one_chunking_message(
            body,
            mock_job_store,
            mock_storage,
            "input-bucket",
...
def test_process_one_chunking_message_invalid_body_returns_false(
    mock_job_store: MagicMock,
    mock_storage: MagicMock,
) -> None:
    result = process_one_chunking_message(
...
def test_process_one_chunking_message_job_not_found_returns_false(
    mock_storage: MagicMock,
) -> None:
    store = MagicMock()
    store.get.return_value = None
    body = _make_s3_event_body("input-bucket", "input/missing-job/source.mp4")
    result = process_one_chunking_message(body, store, mock_storage, "input-bucket")
    assert result is False
