...
3. **Segment processing** — Object-created events for segment files feed a **video-worker queue**. The **video-worker** invokes the **inference backend** (stub, managed GPU endpoint, or HTTP) per segment. Inference reads the segment and writes the result to the output bucket. The video-worker also consumes the **output-events queue** only to **release the inference semaphore** (backpressure); it does not write SegmentCompletions or update job status.
4. **Segment completion** — When the inference backend writes a segment (and, for SageMaker, the async result), S3 sends events to **output-events** and to **job-status-events** (duplicate). The **job-worker** consumes **job-status-events**: **segment file** events (stub/HTTP path) and **SageMaker result** events trigger a **SegmentCompletion** write. So completion is recorded only when the job-worker processes the event (single source of truth).
5. **Reassembly trigger** — After each SegmentCompletion put, the **job-worker** runs **trigger-on-write**: if the job has **status = chunking_complete** and **count(SegmentCompletions) == total_segments**, it conditionally creates a reassembly lock and sends **job_id** to the **reassembly queue**.
6. **Reassembly** — The **job-worker** sets job **status = reassembling** when it sends the job to the reassembly queue. A **media-worker** consumes the reassembly queue: downloads each segment via streaming, builds the concat list from the segment-completion store, runs ffmpeg concat, and uploads **final.mp4** (it does not update job status). If **final.mp4** already exists (idempotent), the media-worker writes a **.reassembly-done** sentinel object so an S3 event is emitted. The **job-worker** consumes job-status-events; when it sees an S3 event for **jobs/\{job_id\}/final.mp4** or **jobs/\{job_id\}/.reassembly-done**, it sets the job to **status = completed** and **completed_at**. So only the job-worker sets **reassembling** and **completed** for this phase.
...
stateDiagram-v2
  ...
  chunking_complete --> reassembling: job-worker sends to reassembly queue
  reassembling --> completed: job-worker sees final.mp4 or .reassembly-done
  completed --> [*]
...
- **reassembling** — Set by the job-worker when it sends the job to the reassembly queue (after all segment completions are in).
- **completed** — Set by the job-worker when it sees a job-status-events S3 notification for **final.mp4** or **.reassembly-done** (media-worker only writes those objects; it does not update job status).
