...
Contains **Terraform outputs** (bucket names, queue URLs, table names, ECR URLs, ALB URL, etc.). It is generated by **`nx run aws-infra:terraform-output`**. Add **`HF_TOKEN`** to the **root** `.env` for SageMaker and use the root **`update-hf-token`** target — see [Bringing the solution up (AWS)](/docs/aws/bring-up).
