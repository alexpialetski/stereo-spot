...
Contains **Terraform outputs** (bucket names, queue URLs, table names, ECR URLs, ALB URL, etc.). It is generated by **`nx run aws-infra:terraform-output`**; you may also add **`HF_TOKEN`** (your Hugging Face token) by hand for SageMaker — see [Bringing the solution up (AWS)](/docs/aws/bring-up) and the **`update-hf-token`** target.
