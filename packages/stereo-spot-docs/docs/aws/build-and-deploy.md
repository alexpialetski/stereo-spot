...
- **stereo-inference:** Built by **AWS CodeBuild** (clone repo, docker build, push to ECR). Trigger: `nx run stereo-inference:inference-build` (requires `aws-infra:terraform-output` and CodeBuild project name in env). When `inference_backend=http`, you build and run your own inference service elsewhere.
...
3. **Update SageMaker** (if `inference_backend=sagemaker` and the inference image changed): run `nx run stereo-inference:inference-redeploy` so the endpoint uses the new image.
...
**Full pipeline:** Terraform apply → build and push all images → `inference-redeploy` if inference image changed → force-new-deployment on ECS services (web-ui, media-worker, video-worker). See [AWS runbooks](/docs/aws/runbooks) for SageMaker update and recovery procedures.
