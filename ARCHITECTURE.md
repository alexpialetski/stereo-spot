...
- `packages/stereocrafter-sagemaker`: Custom **SageMaker inference container** (CUDA, StereoCrafter two-stage pipeline). Model weights are downloaded from Hugging Face at endpoint startup; **Secrets Manager** holds the HF token. The handler receives `s3_input_uri`, `s3_output_uri`, and `mode` (anaglyph | sbs), runs inference, and writes the result in the requested format to the given S3 key. **Build** triggers **AWS CodeBuild** (clones the public repo, builds the image, pushes to ECR); **deploy** updates the SageMaker endpoint to use the new ECR image (new model + endpoint config, then update-endpoint). No local Docker build.
...
2.  **Orchestration (SQS + ECS):**
    ...
    - **Application Auto Scaling** scales the **media-worker** ECS service on chunking + reassembly queue depth and the **video-worker** ECS service on the video-worker queue depth. Workers use **SQS long polling** (WaitTimeSeconds, default 20s via optional env `SQS_LONG_POLL_WAIT_SECONDS`) so messages are picked up as soon as they arrive. GPU inference runs on a **SageMaker** real-time endpoint (e.g. `ml.g4dn.xlarge` or `ml.g5.xlarge`); scale the endpoint or use SageMaker Serverless as needed.
...
5.  **The Video Worker (Thin client + SageMaker):**
    - The video-worker (Fargate) pulls a message, parses the segment payload via shared-types, and builds the canonical output URI `s3://output-bucket/jobs/{job_id}/segments/{segment_index}.mp4`. It calls **SageMaker InvokeEndpoint** with JSON `{"s3_input_uri": "<segment_uri>", "s3_output_uri": "<canonical_output_uri>", "mode": "anaglyph"|"sbs"}` (mode from the job). The **SageMaker endpoint** (custom container running StereoCrafter) reads the segment from S3, runs the two-stage pipeline (depth splatting → inpainting), and **writes the stereo segment in the requested format directly to the given s3_output_uri**. The video-worker does not upload segment bytes; after a successful response it writes the **SegmentCompletion** record to DynamoDB. **Sizing:** Segment processing time is dominated by SageMaker (e.g. a few minutes wall-clock per ~5 min segment on a typical GPU instance); set SQS visibility timeout and endpoint capacity accordingly (see IMPLEMENTATION_PLAN and StereoCrafter docs).
