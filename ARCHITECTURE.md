...
- **Observability:** Use **CloudWatch** for worker logs and metrics; optional **X-Ray** for tracing.
  - **Custom metric:** The SageMaker inference container emits **SegmentConversionDurationSeconds** to namespace **StereoSpot/Conversion** (and dimension **Cloud**, e.g. `aws`) after each segment conversion so you can tune segment size and instance type and predict runtimes.
  - **Analytics pipeline:** Run **`nx run aws-infra:gather-analytics`** (or **`nx run analytics:gather --configuration=aws`**) to pull CloudWatch metrics into a file; maintain **packages/analytics/analytics_history.md** (table of cloud, backend, instance type, avg_duration_sec, eta_seconds_per_mb, date); use **docs/prompts/analyze_analytics.md** with an LLM to compare latest analytics with history and suggest infra changes.
