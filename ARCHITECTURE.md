...
It leverages **ECS (Elastic Container Service)** with **Fargate** for all app workloads (web-ui, media-worker, video-worker) and a **switchable inference backend**: **SageMaker** (default) for GPU inference (StereoCrafter) or **HTTP** (e.g. an EC2 instance in the VPC) for dev/testing; **NX** provides shared logic between your UI and Infrastructure-as-Code (IaC).
...
- `packages/video-worker`: **Thin client** (Python container). Pulls messages from the video-worker queue and invokes inference via a **backend-switchable** path: **stub** (default, for tests), **SageMaker** (real-time endpoint with S3 URIs), or **HTTP** (POST to a base URL, e.g. EC2 in the VPC for dev). The inference side (SageMaker or HTTP server) runs StereoCrafter and writes the stereo segment to the given output key; the video-worker then writes the SegmentCompletion record. Runs on **ECS Fargate** (no GPU). See The Video Worker below.
- `packages/stereocrafter-sagemaker`: Custom **inference container** (CUDA, StereoCrafter two-stage pipeline) used by **SageMaker** or by an **HTTP server** (e.g. on EC2). Model weights are downloaded from Hugging Face at startup; **Secrets Manager** holds the HF token. The handler receives `s3_input_uri`, `s3_output_uri`, and `mode` (anaglyph | sbs), runs inference, and writes the result to the given S3 key. **Build** triggers **AWS CodeBuild** (clone repo, build image, push to ECR); **deploy** is either **SageMaker** (`sagemaker-deploy`) or **EC2** (`stereocrafter-ec2-deploy` when using the HTTP backend). No local Docker build required for SageMaker.
...
**Portability and cloud abstractions:** To avoid vendor lock-in and simplify a future GCP (or other cloud) deployment, use **abstractions from the beginning**. Pipeline logic uses **shared-types** and thin interfaces (e.g. in `packages/shared-types`) for: **job store** (get/put/update job), **segment-completion store** (put completion, query by job ordered by segment_index), **queues** (send/receive messages), and **object storage** (presign upload/download, upload/download). **AWS** implementations live in **packages/aws-adapters** (DynamoDB, SQS, S3). **Compute** is cloud-specific: on AWS we use ECS Fargate for all workloads and a **switchable inference backend** (SageMaker or HTTP/EC2); on GCP you would use Cloud Run and/or GKE with the same container images, plus a GCP equivalent for the inference endpoint (e.g. Vertex AI), and env-based config. Add **GCP** implementations later (e.g. Firestore, Pub/Sub, GCS) behind the same interfaces. App and workers depend on the abstractions and get the implementation by config (e.g. `STORAGE_ADAPTER=aws`). Terraform remains per-cloud; application and worker code stay the same.
...
3.  **S3 event → video-worker path:**
    ...
    - **Duplicate S3 events:** Processing is **idempotent**. The inference side writes output to the deterministic path (`jobs/{job_id}/segments/{segment_index}.mp4`) and the video-worker writes SegmentCompletions once per segment; duplicate events may cause duplicate messages but reprocessing overwrites the same segment output and does not corrupt state.
...
5.  **The Video Worker (Thin client, backend-switchable inference):**
    - The video-worker (Fargate) pulls a message, parses the segment payload via shared-types, and builds the canonical output URI `s3://output-bucket/jobs/{job_id}/segments/{segment_index}.mp4`. Inference is **backend-switchable** via `INFERENCE_BACKEND`: **(1) stub** — local stub (tests); **(2) sagemaker** — **SageMaker InvokeEndpoint** with JSON `{"s3_input_uri", "s3_output_uri", "mode"}`; **(3) http** — **POST** to `INFERENCE_HTTP_URL/invocations` with the same JSON (e.g. EC2 in the VPC for dev/testing). The inference side (SageMaker endpoint or HTTP server running the same custom container) reads the segment from S3, runs the two-stage pipeline (depth splatting → inpainting), and **writes the stereo segment to the given s3_output_uri**. The video-worker does not upload segment bytes; after a successful response it writes the **SegmentCompletion** record to DynamoDB. **Sizing:** Segment processing time is dominated by inference (e.g. a few minutes per ~5 min segment on a typical GPU); set SQS visibility timeout and endpoint/EC2 capacity accordingly (see IMPLEMENTATION_PLAN and StereoCrafter docs).
...
flowchart LR
    ...
    subgraph Build["Build"]
        ...
        CB[CodeBuild: stereocrafter-sagemaker\nclone repo, docker build, push]
    end
    subgraph Push["Push"]
        ECR[(ECR\nsame tag)]
    end
...
2.  **Build and push:** Each package (web-ui, media-worker, video-worker) is packaged into a **Docker image** and pushed to its ECR repo. The **inference container** (StereoCrafter) is built via **AWS CodeBuild**: `nx run stereocrafter-sagemaker:sagemaker-build` triggers CodeBuild to clone the repo, build the image, and push to ECR. **Deploy** depends on the inference backend: **SageMaker** — `nx run stereocrafter-sagemaker:sagemaker-deploy` updates the endpoint; **HTTP (EC2)** — Terraform can create an inference EC2 in the VPC, and `nx run stereocrafter-sagemaker:stereocrafter-ec2-deploy` updates the container on that EC2 via SSM. See [docs/IMPLEMENTATION_PLAN.md](docs/IMPLEMENTATION_PLAN.md) for order of operations. Builds can run **in parallel**. Use a **single image identifier per release** (e.g. git SHA); when using CI, deploy only after all image build/push jobs succeed.
3.  **Deploy ECS and inference:** Task definitions reference ECR image URIs and a tag. Video-worker env is set by Terraform: when `inference_backend=sagemaker` (default), it gets SageMaker endpoint name and region; when `inference_backend=http`, it gets `INFERENCE_HTTP_URL` (from Terraform-managed EC2 private IP or from variable). After building and pushing images, update the **inference** side (SageMaker endpoint or EC2 container as above), then run **force new deployment** on ECS services. Full pipeline: **Terraform apply** → **build + push images** → **update SageMaker endpoint or EC2 inference** (if that image changed) → **force new deployment** on ECS. See [docs/IMPLEMENTATION_PLAN.md](docs/IMPLEMENTATION_PLAN.md) and [docs/RUNBOOKS.md](docs/RUNBOOKS.md) for details.
