...
- `packages/video-worker`: **Thin client** (Python container). Pulls messages from the video-worker queue and invokes inference via a **backend-switchable** path: **stub** (default, for tests), **SageMaker** (real-time endpoint with S3 URIs), or **HTTP** (POST to a base URL you provide). The inference side (SageMaker or HTTP server) runs **iw3** (nunif) and writes the stereo segment to the given output key; the video-worker then writes the SegmentCompletion record. Runs on **ECS Fargate** (no GPU). See The Video Worker below.
...
5.  **The Video Worker (Thin client, backend-switchable inference):**
    - The video-worker (Fargate) pulls a message, parses the segment payload via shared-types, and builds the canonical output URI `s3://output-bucket/jobs/{job_id}/segments/{segment_index}.mp4`. Inference is **backend-switchable** via `INFERENCE_BACKEND`: **(1) stub** — local stub (tests); **(2) sagemaker** — **SageMaker InvokeEndpoint** with JSON `{"s3_input_uri", "s3_output_uri", "mode"}`; **(3) http** — **POST** to `INFERENCE_HTTP_URL/invocations` with the same JSON (your own inference server). The inference side (SageMaker endpoint or HTTP server running the same custom container) reads the segment from S3, runs **iw3** (2D→stereo SBS or anaglyph), and **writes the stereo segment to the given s3_output_uri**. The video-worker does not upload segment bytes; after a successful response it writes the **SegmentCompletion** record to DynamoDB. **Sizing:** Segment processing time is dominated by inference (e.g. a few minutes per ~5 min segment on a typical GPU); set SQS visibility timeout and endpoint/EC2 capacity accordingly (see IMPLEMENTATION_PLAN and StereoCrafter docs).
