...
- Implement: read S3 event from queue; parse input key via shared-types; get job from JobStore; update job to chunking_in_progress; download source to temp; ffmpeg segment (keyframe-aligned, ~50MB/~5min); upload each segment with `segments/{job_id}/{i:05d}_{total:05d}_{mode}.mp4`; single UpdateItem for total_segments + status=chunking_complete.
...
**A/C:**
- [ ] Lambda triggers reassembly only when last segment completes and job is chunking_complete.
- [ ] Conditional create prevents duplicate reassembly messages.
- [ ] Lambda deployment package is built from the same shared-types wheel as other consumers (Nx build dependency and install-from-wheel in CI).
- [ ] Unit tests pass.
